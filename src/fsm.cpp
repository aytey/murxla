...
State::run(RNGenerator& rng)
{
  ...
  /* Disable action if generate() will always return false for this run.
   * This happens e.g., when incremental is disabled and check-sat was already
...
FSM::FSM(RNGenerator& rng,
    ...
    : d_smgr(solver,
      ...
      d_in_untrace_replay_mode(in_untrace_replay_mode),
...
FSM::configure()
{
  ...
  add_action_to_all_states(a_reset, 10000, {}, s_set_logic);
